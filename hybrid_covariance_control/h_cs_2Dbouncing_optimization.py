...
if __name__=='__main__':
    ...
    tmp = epsilon**2 * np.eye(nx1)/4 + sqrtSig0 @ inv_Phi_M1_12 @ SigT_1 @ inv_Phi_M1_12.T @ sqrtSig0
    tmp = (tmp + tmp.T) / 2
    ...
    # ==================== Solve for Pi(t) ====================
    Pi0 = epsilon*invSig0/2 - inv_Phi_M1_12@Phi_M1_11 - sqrtInvSig0@sqrt_tmp@sqrtInvSig0
    ...
    eval_Sig2_0, evec_Sig2_0 = np.linalg.eigh(SigT)
    invSig2_0 = np.linalg.inv(SigT)
    Sig2_T = Sigma_plus

    eval_invSig0, evec_invSig0 = np.linalg.eigh(invSig2_0)
    sqrtInvSig0 = evec_invSig0 @ np.diag(np.sqrt(eval_invSig0)) @ evec_invSig0.T

    sqrtSig2_0 = evec_Sig2_0 @ np.diag(np.sqrt(eval_Sig2_0)) @ evec_Sig2_0.T

    tmp = epsilon**2 * np.eye(nx2)/4 + sqrtSig2_0 @ inv_Phi_M2_12 @ Sig2_T @ inv_Phi_M2_12.T @ sqrtSig2_0
    tmp = (tmp + tmp.T) / 2
    ...
    # ==================== Solve for Pi(t) ====================
    Pi2_0 = epsilon*invSig2_0/2 - inv_Phi_M2_12@Phi_M2_11 + sqrtInvSig0@sqrt_tmp@sqrtInvSig0
    ...
    result = solve_ivp(ode_system_XY_M2, t_span2, v_XY_M2_0_flat, method='RK23', t_eval=t_eval2)
    ...
    Sigma_minus_ellipse_boundary, ax2 = plot_2d_ellipsoid_boundary(states[t_event], Sig_minus, ax2, 'k', linewidth=5.0)
    Sigma_plus_ellipse_boundary, ax2 = plot_2d_ellipsoid_boundary(states[t_event+1], Sigma_plus, ax2, 'c', linewidth=5.0)
